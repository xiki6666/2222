...
local noclipKeybindButton = Instance.new("TextButton")
...
local noclipKeybindListening = false
...
styleButton(noclipButton, "Enable NoClip", UDim2.new(0, 10, 0, 420))
styleButton(noclipKeybindButton, "NoClip Key: Z", UDim2.new(0, 10, 0, 460))
...
local function disableNoClip()
	...
	-- Восстанавливаем коллизию для всех частей персонажа
	for _, part in pairs(character:GetDescendants()) do
		if part:IsA("BasePart") then
...
local function toggleNoClip()
	if noclipEnabled then
		...
		enableNoClip()
		-- Постоянно отключаем коллизию для новых частей
...
-- Функция для настройки клавиши NoClip
local function setNoClipKeybind()
	noclipKeybindListening = true
	noclipKeybindButton.Text = "Press any key..."

	local connection
	connection = game:GetService("UserInputService").InputBegan:Connect(function(input, gameProcessed)
		if noclipKeybindListening and not gameProcessed then
			if input.UserInputType == Enum.UserInputType.Keyboard then
				noclipKey = input.KeyCode
				noclipKeybindButton.Text = "NoClip Key: " .. tostring(input.KeyCode):gsub("Enum.KeyCode.", "")
				noclipKeybindListening = false
...
noclipKeybindButton.MouseButton1Click:Connect(setNoClipKeybind)
...
local function restoreMenuOnDeath()
	...
	player.CharacterAdded:Connect(function(character)
		...
		-- Восстанавливаем состояние NoClip после смерти
		if noclipEnabled then
			enableNoClip()
		end
	end)
