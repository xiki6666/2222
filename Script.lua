...
local menuKey = Enum.KeyCode.U  -- Клавиша для скрытия/показа меню
...
game:GetService("UserInputService").InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed then
        ...
        elseif input.KeyCode == menuKey then  -- Клавиша U для скрытия/показа меню
            toggleMenu()
        end
    end
end)
...
-- Обновление ESP при смене команды
for _, player in pairs(game.Players:GetPlayers()) do
